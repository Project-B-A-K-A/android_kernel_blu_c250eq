...
extern int drm_shutdown;
extern int ilitek_init;
extern void ili_tp_rst_low(void);
extern int ili_sleep_handler(int mode);
extern void fts_suspend(void);
extern void fts_resume(void);
...
#ifdef LCD_BIAS_CARON

static int enable_lcdbias(bool en);
#endif
...
#ifdef LCD_BIAS_CARON
static struct i2c_client *lcdbias_client;

static inline int lcdbias_smbus_read_byte(struct i2c_client *client,
		unsigned char reg_addr, unsigned char *data)
{
	signed int dummy = 0;
	dummy = i2c_smbus_read_byte_data(lcdbias_client, reg_addr);
	if (dummy < 0)
		return dummy;
...
static inline int lcdbias_smbus_write_byte(struct i2c_client *client,
		unsigned char reg_addr, unsigned char data)
{
	signed int dummy = 0;

	dummy = i2c_smbus_write_byte_data(lcdbias_client, reg_addr, data);
	if (dummy < 0)
		return dummy;
	return 0;
}
...
static int enable_lcdbias(bool en)
{
	...
	lcdbias_smbus_write_byte(lcdbias_client, 0x00, 0xf);
	lcdbias_smbus_write_byte(lcdbias_client, 0x01, 0xf);
	lcdbias_smbus_read_byte(lcdbias_client, 0x00, &t1);
	lcdbias_smbus_read_byte(lcdbias_client, 0x01, &t2);
	printk( "%s called lcdbias_client->addr=%x t1=%x, t2=%x\n", __func__, lcdbias_client->addr, t1,t2);
	return 0;
}

static int  lcdbias_i2c_probe(struct i2c_client *client,
				       const struct i2c_device_id *id)
{
	printk( "%s called\n", __func__);
	lcdbias_client = client;
	return 0;
}

static int  lcdbias_i2c_remove(struct i2c_client *client)
{
	return 0;
}


static struct of_device_id lcdbias_of_match_table[] = {
       { .compatible = "caron,lcdbias", },
	{ },
};

MODULE_DEVICE_TABLE(of, lcdbias_of_match_table);

static const struct i2c_device_id lcdbias_id[] = {
	{"lcdbias", 0},
	{ }
};

static struct i2c_driver lcdbias_driver = {
	.driver = {
		.name = "lcdbias",
#ifdef CONFIG_OF		
		.of_match_table = lcdbias_of_match_table,		
#endif				
		.owner = THIS_MODULE,
	},
	.probe    = lcdbias_i2c_probe,
//	.remove   = __devexit_p(lcdbias_i2c_remove),
	.remove   = lcdbias_i2c_remove,
	//.suspend  = lcdbias_i2c_suspend,
	//.resume   = lcdbias_i2c_resume,

	.id_table = lcdbias_id,
};

static int __init lcdbias_i2c_init(void)
{
	
	printk( "%s caikai called init\n", __func__);

	return i2c_add_driver(&lcdbias_driver);
}

static void __exit lcdbias_i2c_exit(void)
{
	printk( "%s called\n", __func__);
	

	i2c_del_driver(&lcdbias_driver);
}


module_init(lcdbias_i2c_init);
#endif
