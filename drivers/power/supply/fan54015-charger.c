...
#define FAN54015_REG_TERMCUR_MASK				GENMASK(2, 0)
...
int ac_usb_type = POWER_SUPPLY_USB_TYPE_UNKNOWN;
...
static struct fan54015_charger_info *fan54015_info=NULL;
...
static int fan54015_charger_hw_init(struct fan54015_charger_info *info)
{
	...
	if (ret) {
		...
		ret = fan54015_update_bits(info, FAN54015_REG_1,
					   FAN54015_REG_TERMINAL_CURRENT_MASK, 0x08);
		if (ret) {
...
static void fan54015_charger_work(struct work_struct *data)
{
	...
	if (info->limit > 0 && !info->charging && present) {
		...
		case SDP_TYPE:
			...
			ac_usb_type = POWER_SUPPLY_USB_TYPE_SDP;
			//printk("fan54015_charger_work 1::ac_usb_type = 0x%x\n",ac_usb_type);
			break;
		case DCP_TYPE:
			limit_cur = info->cur.dcp_limit;
			cur = info->cur.dcp_cur;
			ac_usb_type = POWER_SUPPLY_USB_TYPE_DCP;
			//printk("fan54015_charger_work 2::ac_usb_type = 0x%x\n",ac_usb_type);
			break;
		case CDP_TYPE:
			limit_cur = info->cur.cdp_limit;
			cur = info->cur.cdp_cur;
			ac_usb_type = POWER_SUPPLY_USB_TYPE_CDP;
			//printk("fan54015_charger_work 3::ac_usb_type = 0x%x\n",ac_usb_type);
			break;
		default:
			limit_cur = info->cur.unknown_limit;
			cur = info->cur.unknown_cur;
			ac_usb_type = POWER_SUPPLY_USB_TYPE_UNKNOWN;
			//printk("fan54015_charger_work 4::ac_usb_type = 0x%x\n",ac_usb_type);
		}
					     ...
					     union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENERGY_FULL_DESIGN:
		ret = fan54015_read(fan54015_info, FAN54015_REG_0, &reg_val);
		if(((reg_val>>4)&0x03)==0x02)
			val->intval = 1;
		else
			val->intval = 0;
		break;
...
extern void cm_report_ovp(void);
u32 statgpio=0;
int get_statgpio(void)
{
	u8 reg_val;
	int ret;
	...
	if (ret < 0)
	{
		printk(KERN_EMERG"====fan54015 ovp read FAN54015_REG_0 fail = %d====\n", ret);
		return ret;
	}
#if 0	
	printk(KERN_EMERG"====fan54015 ovp read FAN54015_REG_0 reg_val = %x====\n", reg_val);

	fan54015_read(fan54015_info, FAN54015_REG_1, &reg_val);
	printk(KERN_EMERG"====fan54015 read FAN54015_REG_1 reg_val = %x====\n", reg_val);
	fan54015_read(fan54015_info, FAN54015_REG_2, &reg_val);
	printk(KERN_EMERG"====fan54015 read FAN54015_REG_2 reg_val = %x====\n", reg_val);	
	fan54015_read(fan54015_info, FAN54015_REG_3, &reg_val);
	printk(KERN_EMERG"====fan54015 read FAN54015_REG_3 reg_val = %x====\n", reg_val);
	fan54015_read(fan54015_info, FAN54015_REG_4, &reg_val);
	printk(KERN_EMERG"====fan54015 read FAN54015_REG_4 reg_val = %x====\n", reg_val);
	fan54015_read(fan54015_info, FAN54015_REG_5, &reg_val);
	printk(KERN_EMERG"====fan54015 read FAN54015_REG_5 reg_val = %x====\n", reg_val);
	fan54015_read(fan54015_info, FAN54015_REG_6, &reg_val);
	printk(KERN_EMERG"====fan54015 read FAN54015_REG_6 reg_val = %x====\n", reg_val);
#endif	
	ret = gpio_get_value(statgpio);
//	printk(KERN_EMERG"====fan54015 ovp read statgpio ret = %x====\n", ret);
	if((((reg_val>>4)&0x03)==0)&&ret)
		return 1;

	return 0;
}
#if 0
static irqreturn_t fan54015_plat_isr_top_half(int irq, void *dev_id)
{
	if (irq != fan54015_info->stat_irq) {
		printk(KERN_EMERG"====fan54015 Incorrect irq number (%d)====\n", irq);
		return IRQ_NONE;
	}
//return IRQ_HANDLED;
...
static irqreturn_t fan54015_plat_isr_bottom_half(int irq, void *dev_id)
{
	u8 reg_val;
	int ret;
		...
		const struct i2c_device_id *id)
{
	...
	fan54015_info = info = devm_kzalloc(dev, sizeof(*info), GFP_KERNEL);
	if (!info)
		return -ENOMEM;
	info->client = client;
	info->dev = dev;
	...
	statgpio = stat_gpio = of_get_named_gpio_flags(client->dev.of_node, "stat-gpios", 0, &flag);
	if (!gpio_is_valid(stat_gpio)) {
		dev_err(dev,"Invalid INT gpio: %d\n", stat_gpio);
		return -EBADR;
	}
	ret = gpio_request(stat_gpio, "FAN54015_STAT_INT");
	if (ret < 0) {
		dev_err(dev, "Request IRQ GPIO failed, ret = %d\n", ret);
		gpio_free(stat_gpio);
		ret = gpio_request(stat_gpio, "FAN54015_STAT_INT");
		if (ret < 0) {
			dev_err(dev,"Retrying request INT GPIO still failed , ret = %d\n", ret);
			return ret;
		}
	}

#if 0	
	info->stat_irq  = gpio_to_irq(stat_gpio);

			  
	printk("fan54015 irq_num = %d\n", info->stat_irq);
			  
	ret = devm_request_threaded_irq(info->dev, info->stat_irq,
				 fan54015_plat_isr_top_half,
				 fan54015_plat_isr_bottom_half,
				 IRQF_TRIGGER_RISING | IRQF_ONESHOT, "fan54015", NULL);
#endif
	return 0;
}
