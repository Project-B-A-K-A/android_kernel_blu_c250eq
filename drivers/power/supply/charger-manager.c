...
#define CHECK_AC_USB_CHARGER
#ifdef  CHECK_AC_USB_CHARGER
extern int ac_usb_type;
#endif
...
static bool is_full_charged(struct charger_manager *cm)
{
	...
	union power_supply_propval val;
	struct power_supply *fuel_gauge,*psy;
	bool is_full = false;
	int ret = 0;
	int uV, uA;
	int i;
	...
	for (i = 0; cm->desc->psy_charger_stat[i]; i++) {
		...
		power_supply_put(psy);
		if (ret) {
			dev_err(cm->dev, "power_supply_get_property POWER_SUPPLY_PROP_CHARGE_FULL failed\n");
			continue;
		}
...
static struct charger_manager *scm = NULL;
#if 0
void cm_report_ovp(void)
{
...
//	dev_info(scm->dev, "Charging voltage is larger than %d\n",
//		 desc->charge_voltage_max);
	printk(KERN_EMERG"=======fan54015 cm_report_ovp=======\n");
	if(scm==NULL)
		return;
	desc = scm->desc;

	if (scm->charging_status != 0 &&
	    !(scm->charging_status & CM_CHARGE_VOLTAGE_ABNORMAL))
		return ;
	...
	printk(KERN_EMERG"=======fan54015 cm_report_ovp charge_vol =%d=======\n",charge_vol);
	if(!charge_vol)
		return;
	
	printk(KERN_EMERG"=======fan54015 cm_report_ovp+++=======\n");
	uevent_notify(scm, "Discharging");
	try_charger_enable(scm, false);
	scm->charging_status |= CM_CHARGE_VOLTAGE_ABNORMAL;
	power_supply_changed(scm->charger_psy);

}
#endif
extern int get_statgpio(void);
static int cm_check_charge_voltage(struct charger_manager *cm)
{
	...
	charge_vol = val.intval;
dev_info(cm->dev,"fan54015 cm_check_charge_voltage charge_vol=%d desc->charge_voltage_max=%d get_statgpio=%d\n",charge_vol,desc->charge_voltage_max, get_statgpio());
	if (cm->charger_enabled &&((charge_vol > desc->charge_voltage_max) || (get_statgpio()==1&&charge_vol>0))) {
...
{
	  //printk("cm_update_charger_type_status!!!\n");
    if (is_ext_pwr_online(cm)) {
    	  //printk("cm_update_charger_type_status1::ac_usb_type = 0x%x,POWER_SUPPLY_USB_TYPE_DCP = 0x%x\n",ac_usb_type, POWER_SUPPLY_USB_TYPE_DCP);
        if (ac_usb_type == POWER_SUPPLY_USB_TYPE_DCP) {
            usb_main.USB_ONLINE = 0;
            ac_main.AC_ONLINE = 1;
        } else {
        	  //printk("cm_update_charger_type_status2::ac_usb_type = 0x%x,POWER_SUPPLY_USB_TYPE_SDP = 0x%x\n",ac_usb_type, POWER_SUPPLY_USB_TYPE_SDP);
            ac_main.AC_ONLINE = 0;
