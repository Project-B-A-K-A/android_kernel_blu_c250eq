...
#include "ilitek9882x.h"

#define DTS_INT_GPIO	"touch,irq-gpio"
#define DTS_RESET_GPIO	"touch,reset-gpio"
#define DTS_OF_NAME	"tchip,ilitek9882"

int ilitek_init = 0;

void ili_tp_rst_low(void)
{
	if(ilitek_init)
...
#if 1
static ssize_t tp_version_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "fw=0x%x\n", ilits->chip->fw_ver);
}
static ssize_t tp_version_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
#if 0
...
static DEVICE_ATTR_RW(tp_version);

static struct attribute *ilitek_dev_version_atts[] = {
	&dev_attr_tp_version.attr,
	NULL
};

static const struct attribute_group ilitek_dev_version_atts_group = {
	.attrs = ilitek_dev_version_atts,
};

static const struct attribute_group *ilitek_dev_attr_groups[] = {
	&ilitek_dev_version_atts_group,
	NULL
};
...
{
	ILI_INFO("ilitek_pm_suspend\n");
	return 0;
//  return ili_sleep_handler(TP_SUSPEND);
}

static int ilitek_pm_resume(struct device *dev)
{
	ILI_INFO("ilitek_pm_resume\n");  
	return 0;
...
static const struct dev_pm_ops ilitek_pm_ops = {
	SET_RUNTIME_PM_OPS(ilitek_pm_suspend, ilitek_pm_resume, NULL)
...
{
	ILI_INFO("ILITEK TP driver init for UNISOC\n");
